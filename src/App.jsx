...
export default function App() {
  ...
  // Trend data: rolling 7-day avg rank per player per game
  const trendData = useMemo(()=>{
    const WINDOW = 7;
    const result = [];
    for (let i = WINDOW-1; i < allDays.length; i++) {
      const window = allDays.slice(i-WINDOW+1, i+1);
      const point = { date: dateLabel(allDays[i].date) };
      PLAYERS.forEach(p=>{
        LEADERBOARD_GAMES.forEach(g=>{
          const avg = window.reduce((s,d)=>s+(d.ranks[g][p]||DEFAULT_RANK),0)/WINDOW;
          point[`${p}_${g}`] = parseFloat(avg.toFixed(2));
        });
        const dayAvg = window.reduce((s,d)=>s+d.lb.totals[p],0)/WINDOW;
        point[`${p}_total`] = parseFloat(dayAvg.toFixed(1));
      });
      result.push(point);
    }
    return result;
  ...
  return(<>
    ...
    <div className="app">
      ...
      <div className="body">
        ...
        {/* ══ TRENDS ════════════════════════════════════════════════════════ */}
        {tab==="trends" && (<>
          <div className="eyebrow">7-day rolling average rank · lower = better · from {dateLabel(allDays[6]?.date)}</div>

          {/* Overall trend */}
          <div className="chart-wrap section">
            <div className="chart-title">Overall daily points <span className="chart-sub">(rolling 7-day avg)</span></div>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={trendData} margin={{top:4,right:8,bottom:4,left:-20}}>
                <XAxis dataKey="date" tick={{fontSize:10,fill:"#bbb"}} tickLine={false} axisLine={false} interval={Math.floor(trendData.length/6)}/>
                <YAxis tick={{fontSize:10,fill:"#bbb"}} tickLine={false} axisLine={false} reversed={false}/>
                <Tooltip contentStyle={{background:"#111",border:"none",borderRadius:8,fontSize:11,color:"#f0efe9"}} labelStyle={{color:"#aaa",marginBottom:4}}/>
                {PLAYERS.map(p=>(
                  <Line key={p} type="monotone" dataKey={`${p}_total`} name={p}
                    stroke={PLAYER_META[p].color} strokeWidth={2} dot={false} strokeOpacity={0.9}/>
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>

          {/* Per-game trend charts */}
          {LEADERBOARD_GAMES.map(g=>(
            <div key={g} className="chart-wrap" style={{marginBottom:14}}>
              <div className="chart-title">
                <span className="chip" style={{background:GAME_META[g].color,marginRight:8}}>{GAME_META[g].abbr}</span>
                {g} avg rank <span className="chart-sub">(rolling 7-day · lower = better)</span>
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={trendData} margin={{top:4,right:8,bottom:4,left:-20}}>
                  <XAxis dataKey="date" tick={{fontSize:10,fill:"#bbb"}} tickLine={false} axisLine={false} interval={Math.floor(trendData.length/6)}/>
                  <YAxis tick={{fontSize:10,fill:"#bbb"}} tickLine={false} axisLine={false} domain={[1,5]} reversed={false}/>
                  <Tooltip contentStyle={{background:"#111",border:"none",borderRadius:8,fontSize:11,color:"#f0efe9"}} labelStyle={{color:"#aaa",marginBottom:4}}/>
                  {PLAYERS.map(p=>(
                    <Line key={p} type="monotone" dataKey={`${p}_${g}`} name={p}
                      stroke={PLAYER_META[p].color} strokeWidth={2} dot={false} strokeOpacity={0.9}/>
                  ))}
                </LineChart>
              </ResponsiveContainer>
